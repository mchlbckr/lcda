...
- wmk.start:
  ...
  A matrix of starting values for the parameter \\w\_{mk}\\ (see
  details). The default is `NULL`, producing random starting values.
  Note that if `nrep>1`, then any user-specified `wmk.start` values are
  only used in the first of the nrep attempts.
...
- nrep:
  ...
  log-likelihood function. `cclcda2` uses the parameter estimates
...
- na.rm:
  ...
  Logical, for how `cclcda2` handles cases with missing values on the
...
The `cclcda2`-function performs a Common Components Latent Class
Discriminant Analysis 2 (CCLCDA2). The class conditional model to
estimate is \$\$f_k(x) = \sum\_{m=1}^{M_k} w\_{mk}
\prod\_{d=1}^D\prod\_{r=1}^{R_d} \theta\_{mdr}^{x\_{dr}},\$\$ where
\\m\\ is the latent subclass index, \\d\\ is the variable index and
\\r\\ is the observation index. The variable \\x\_{dr}\\ is \\1\\ if the
variable \\d\\ of this observation is \\r\\. This Latent Class Modell
will be estimated. The class conditional mixing proportions \\w\_{mk}\\
and the parameters \\\theta\_{mdr}\\ are computed in every step of the
EM-Algorithm.
...
Additionally `cclcda2` provides quality criteria which should give
...
object
#> Call:
#> cclcda2.default(x = data, grouping = grouping, m = 4)
#> 
#> Number of classes:  2 
#> 
#> Prior probabilites:
